...
<template>
  ...
  >
    ...
    >
      <div class="relative space-y-5 pb-6">
        <div
          v-for="(step, stepIndex) in renderedSteps"
          :key="step.key"
          class="agent-runtime-demo__row grid grid-cols-[44px_minmax(0,1fr)] gap-3"
        >
          <div class="relative flex h-full justify-center pt-1">
            <div
              class="agent-runtime-demo__icon z-10 flex h-8 w-8 items-center justify-center rounded-xl border shadow-sm"
              :class="step.type === 'user'
                ? 'border-blue-200/90 bg-blue-600 text-white shadow-[0_12px_22px_rgba(37,99,235,0.18)]'
                : 'border-slate-900/10 bg-white text-slate-800 shadow-[0_10px_20px_rgba(148,163,184,0.1)]'"
            >
              <svg
                v-if="step.type === 'user'"
              ...
              <svg
                v-else
                viewBox="0 0 24 24"
                class="h-[16px] w-[16px] fill-current"
                xmlns="http://www.w3.org/2000/svg"
                aria-hidden="true"
              >
            ...
            <div
              v-if="stepIndex < renderedSteps.length - 1"
              class="absolute top-9 bottom-[-20px] left-1/2 w-px -translate-x-1/2 bg-gradient-to-b from-blue-200/95 via-slate-200 to-slate-100"
            />
          </div>

          <div v-if="step.type === 'user'" class="agent-card agent-card--user">
            <div class="mb-3 flex items-center gap-2.5 px-0.5">
              <p class="text-[11px] font-bold tracking-[0.16em] text-blue-600">
                {{ step.label }}
              </p>
              <span class="h-2.5 w-2.5 rounded-full bg-blue-500 shadow-[0_0_0_6px_rgba(59,130,246,0.08)]" />
            </div>
            <div class="whitespace-pre-wrap text-[14px] leading-[1.72] text-slate-700 font-mono">
              {{ step.content }}
            </div>
          </div>
...
.agent-runtime-demo :deep(.streaming-chat--embedded .message-header) {
  margin-bottom: 12px;
}

.agent-runtime-demo :deep(.streaming-chat--embedded .message-header span) {
  letter-spacing: 0.16em;
}

/* Match TOT: gray transparent thinking block */
.agent-runtime-demo :deep(.streaming-chat--embedded .thinking-block > div) {
  background: rgba(241, 245, 249, 0.55);
  border-color: rgba(226, 232, 240, 0.5);
  box-shadow: none;
}

/* Match TOT: transparent border, translucent glass */
.agent-runtime-demo :deep(.streaming-chat--embedded .main-response) {
  border-radius: 22px;
...
.agent-runtime-demo :deep(.streaming-chat--embedded .tool-call-shell) {
  border-radius: 20px;
}

.agent-runtime-demo :deep(.streaming-chat--embedded .tool-result-shell) {
  box-shadow:
    inset 0 1px 0 rgba(255, 255, 255, 0.72),
    0 8px 20px rgba(148, 163, 184, 0.04);
}
